...
- **`reference/`** — **open-source CRM/ERP study refs** (gitignored; **Odoo / SuiteCRM / ERPNext**). READ-ONLY inspiration for data models / workflow engines / feature breadth — **research subagents should grep these BEFORE web research**. See `reference/README.md` for the module→roadmap map. Never copy code (different stacks + AGPL/GPL/LGPL); translate patterns into our TS/React/Supabase.

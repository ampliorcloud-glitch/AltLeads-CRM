...
const TICKETS = [
  ...
  ...((() => {
    ...
    return [
      ...
      V('ALT-373','FE PII hardening — no PII cached in drafts / search index / exports / filters','Security','P2','Planned',
        'Security audit (Cycle 6) found 4 FE-fixable PII leaks (no DB needed): (1) useUnsavedChanges draft cache stores email/mobile in localStorage; (2) globalSearch in-memory haystack holds email/phone until logout; (3) ExportButton exports email/mobile with no redaction option; (4) ALT-369 list-filter persistence includes the free-text "search" field which could capture a typed phone/email. Fix: strip PII before caching/persisting, add export redaction opt-in, exclude search term from persisted filters. Build next cycle.'),

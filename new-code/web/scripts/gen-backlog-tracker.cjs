...
const TICKETS = [
  ...
  ...((() => {
    ...
    return [
      ...
      V('ALT-429','Companies: load full per-project Status & Owner for sort/filter/export','Bug','P1','In Progress',
      ...
      V('ALT-434','Fix magic-FK address corruption (createLead address_id ?? 1; createClient borrows address)','Bug','P1','In Progress',
        'DATA-CORRUPTION (O5/D8): createLead falls back to address_id ?? 1 and createClient borrows another client address_id to satisfy a NOT-NULL FK — every fallback lead/client is stamped with a wrong hard-coded foreign address, polluting city reporting/filters. DONE for createLead/updateLead (removed the ?? 1 fallback → null when no city, address_id is nullable). createClient HELD: its fix inserts a null-city address but address_master.city_id nullability is unverified + client_association.address_id is NOT NULL — verify the schema (or expose city in the client form) before changing the working create-client path.'),
      V('ALT-435','Stop rendering load failures as "no data"; explain hidden bulk buttons','Bug','P1','In Progress',
      ...
      V('ALT-450','Multi-project export for companies/contacts (status/owner/notes per project)','Feature','P2','Backlog',

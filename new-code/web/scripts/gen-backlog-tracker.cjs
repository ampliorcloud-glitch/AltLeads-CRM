...
const TICKETS = [
  ...
  ...((() => {
    ...
    return [
      ...
      V('ALT-400','Recycle bin — list + restore soft-deleted records','Feature','P1','Backlog',
      ...
      V('ALT-416','Atomic record-merge (SECURITY DEFINER RPC) — fix non-atomic merge','Bug','P1','Backlog',
      ...
      V('ALT-428','Surface silent truncation (Meetings 2000-cap, .limit(5000) facets, Cmd-K index)','Bug','P1','Backlog',
      ...
      V('ALT-429','Companies: load full per-project Status & Owner for sort/filter/export','Bug','P1','Backlog',
        'LAUNCH-BLOCKER (L4/L5): Account Status & Owner load for the CURRENT PAGE only (sparse statusMap), so sorting/grouping/filtering and EXPORT of those columns are blank/wrong for unvisited pages. The export meant to be the re-import template ships blank status for ~every row. Load the full per-project status/owner set for the filtered list. Buildable now.'),
      V('ALT-430','Optimistic-concurrency / lost-update guard on record writes','Bug','P1','Backlog',
        'LAUNCH-BLOCKER (C1/C2): every update() is last-writer-wins with no version/updated_date precondition — two users on the same migrated account silently clobber each other. Add an updated_date/version check on the core master updates; surface a friendly "this record changed since you opened it" conflict. Buildable now (per-write).'),
      ...
      V('ALT-432','Remove false "recoverable by an admin" promise on Merge (until restore exists)','Bug','P1','Backlog',
      ...
      V('ALT-433','Resolve lead two-owner conflict (lead_report.user_id vs created_by)','Bug','P1','Backlog',
      ...
      V('ALT-434','Fix magic-FK address corruption (createLead address_id ?? 1; createClient borrows address)','Bug','P1','Backlog',
        'DATA-CORRUPTION (O5/D8): createLead falls back to address_id ?? 1 and createClient borrows another client address_id to satisfy a NOT-NULL FK — every fallback lead/client is stamped with a wrong hard-coded foreign address, polluting city reporting/filters. Create/resolve the address properly. Buildable now (validate write).'),
      V('ALT-435','Stop rendering load failures as "no data"; explain hidden bulk buttons','Bug','P1','Backlog',
        'TRUST (L18/L24/H3/L20): catch->return [] across contacts/companies/globalSearch/leadsApi and per-chunk continue paint blank status that is actually a load FAILURE; bulk buttons silently vanish when no project is selected. Distinguish empty vs error (+ Retry), and show why an action is unavailable. Buildable now.'),
      V('ALT-436','Bulk-selection safety (clear on filter change; select-all respects true set; max guard)','Bug','P1','Backlog',
      ...
      V('ALT-437','List display fixes: numeric/date sort + show meeting time & lead stage','Bug','P2','Backlog',
      ...
      V('ALT-440','Default sort + user-pinned columns per view','Feature','P2','Backlog',
      ...
      V('ALT-442','Associations across modules (link extra company/contact/lead/meeting)','Feature','P1','Backlog',
        'Design spec complete: record_association table (canonical-ordered, symmetric) + RLS + data/associations.ts merging direct-FK relations with extra associations + "Associated records" panel & picker on detail pages. Phase 2 of the design. Mostly buildable; confirm who-may-associate.'),
      ...
      V('ALT-449','WishlistPage parity (preview panel + bulk reassign) + EditableGrid save toast','Bug','P2','Backlog',

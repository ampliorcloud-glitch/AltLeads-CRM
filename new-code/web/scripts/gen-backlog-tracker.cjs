...
const TICKETS = [
  ...
  ...((() => {
    ...
    return [
      ...
      V('ALT-374','Delight wave 1 — press "?" keyboard-help overlay + actionable empty states','Feature','P2','In Progress',
        'UX-DELIGHT-BACKLOG Tier-1 (Cycle 7, QC PASS). Press "?" (when not typing) opens a focus-trapped overlay listing the app\'s REAL shortcuts (Cmd/Ctrl-K, arrows, Enter/Space to open a row, Esc) — no invented bindings; mounted once at app root. New reusable EmptyState component; LeadsPage empty state now offers a next action ("Clear filters" / friendly message) instead of bare text — reference impl; fan out to the other list pages next. Pure FE. Not pushed.'),

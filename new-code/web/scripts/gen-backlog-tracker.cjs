...
const TICKETS = [
  ...
  ...((() => {
    ...
    return [
      ...
      V('ALT-401','Progress bar + cancel on bulk operations','Feature','P1','Backlog',
        'BUILDABLE NOW (pure FE): bulk ops are sequential await loops with only a spinner. Thread onProgress + AbortSignal through bulkActions.ts/assignment.ts; render a progress bar + cancel between rows.'),

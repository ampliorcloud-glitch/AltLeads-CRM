...
function uxAuditTickets() {
  ...
  const PROGRESS = {
    ...
    'ALT-215': { status: 'In Progress', finished: null, extra: ' 🔧 IN PROGRESS 2026-06-21: shipped #1 (truncation tooltips on Leads/Companies/Contacts name cells), #3 (bell → /notifications + unread badge + aria-label), #4 (hide create from non-admins), #6 (search-clear ×), #7 (copy-to-clipboard + mailto/tel on contact email/phone via CopyButton), #9 (surface swallowed status/toggle errors), #13 (Contacts 1000-row cap), #14 (modal Esc via shared Modal); plus DispositionForm "Call logged" + wishlist convert success toasts. Remaining: #2,5,8,10,11,12.' },
  };

...
function uxAuditTickets() {
  ...
  const PROGRESS = {
    ...
    'ALT-183': { status: 'In Progress', finished: null, extra: ' 🔧 IN PROGRESS 2026-06-21 (owner #1 ask): new reusable MultiSelectFilter (searchable popover + checkboxes + chip count, OR-within-facet, empty=all) replaces the single-value native selects on Leads (Agent/Project/City/Source/Industry/Stage), Contacts (Company/City) and Companies (Industry/City). Remaining: Meetings + Wishlist lists + per-column/advanced operators (Top#7, ALT-184).' },
    ...
    'ALT-215': { status: 'In Progress', finished: null, extra: ' 🔧 IN PROGRESS 2026-06-21: shipped #1 (truncation tooltips on Leads/Companies/Contacts name cells), #3 (bell → /notifications + unread badge + aria-label), #4 (hide create from non-admins), #9 (surface swallowed status/toggle errors), #13 (Contacts 1000-row cap); plus DispositionForm "Call logged" + wishlist convert success toasts. Remaining: #2,5,6,7,8,10,11,12,14.' },
  };
